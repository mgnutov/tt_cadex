...
TEST(Circle, getPoint) {
  tt::Circle c(1.5);
  EXPECT_EQ(c.getPoint(M_PI_2), tt::Coordinates({0, 1.5, 0}));
}

TEST(Circle, getDerivative) {
  tt::Circle c(1.5);
  EXPECT_EQ(c.getDerivative(M_PI_2), tt::Coordinates({-1.5, 0, 0}));
}

TEST(Circle, getRadius) {
  tt::Circle c(1.5);
  EXPECT_EQ(c.getRadius(), 1.5);
}
...
TEST(Ellipse, getPoint) {
  ...
  EXPECT_EQ(e.getPoint(M_PI_2), tt::Coordinates({0, 1, 0}));
}

TEST(Ellipse, getDerivative) {
  tt::Ellipse e(1.5, 1.0);
  EXPECT_EQ(e.getDerivative(M_PI_2), tt::Coordinates({-1.5, 0, 0}));
}
...
TEST(Helixe, getPoint) {
  tt::Helixe e(1.5, 2.5);
  double t = M_PI_2;
  EXPECT_EQ(e.getPoint(t), tt::Coordinates({0, 1.5, 2.5 * t / (2 * M_PI)}));
}

TEST(Helixe, getDerivative) {
  tt::Helixe e(1.5, 2.5);
  double t = M_PI_2;
  EXPECT_EQ(e.getDerivative(t), tt::Coordinates({-1.5, 0, 2.5 / (2 * M_PI)}));
}
